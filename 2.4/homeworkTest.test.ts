...
const driver: WebDriver = new Builder()
    ...
    .build();
//Does one ever run into the problem where they cant find a locator or it doesnt exisit.
...
describe("Employee Manager 1.2", () => {
    ...
    //Describe is like our test plan and it is like our test case
    describe("handles error messages correctly", () => {
        ...
        test("lets you cancel out of an error message", async () => {
            ...
            */
            ...
            await driver.findElement(nameInput).clear();
            // Wondering what other speacial features the cursor and keyboard can do
            await driver.findElement(nameInput).sendKeys(Key.SPACE, Key.BACK_SPACE);
            await driver.findElement(saveButton).click();
            await driver.wait(until.elementLocated(errorCard));
            expect(await (await driver.findElement(errorCard)).getText()).toBe(
            ...
            //sometomes in the past my test suites have failed due to the await sleep when i comment the function out it ends up passing
